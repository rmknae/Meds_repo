...
map:
    ...
    sw s0, 8(sp)       # Save s0 (caller-saved)
    sw s1, 4(sp)       # Save s1 (caller-saved)
    ...
    ##answer: The value is returned in a0.
    ...
    ##answer: Because map expects the function pointer as its second 
         ##argument in a1. Register a1 may have been overwritten during the
        ## function call, so it must be restored.
    ### YOUR CODE HERE ###
    add a1, s1, x0 
    # Recurse
    ### YOUR CODE HERE ###
    ...
    lw s0, 8(sp)       # Save s0 (caller-saved)
    lw s1, 4(sp) 
    addi sp, sp, 16  
